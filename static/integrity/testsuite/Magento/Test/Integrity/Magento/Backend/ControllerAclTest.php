...
class ControllerAclTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testAcl()
    {
        ...
        $changedFiles = Files::init()->getPhpFiles();
        foreach ($changedFiles as $line) {
            ...
            if (strpos($relativeFilePath, '/vendor/')) {
                continue;
            }
    ...
    {
        if (preg_match('~(Controller\/Adminhtml\/.*)\.php~', $relativeFilePath, $matches)) {
            if (count($matches) === 2) {
                $partPath = $matches[1];
                return $partPath;
            }
    ...
    {
        $className = str_replace('/', '\\', $this->moduleNameSpace.$controllerPath);
        try {
