...
class ExtensibleInterfacesTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $codeInterfaceFiles = $this->getFiles(BP , '*Interface.php');
        $interfaces = [];
        $filesToCheck = $this->blacklistFilter(array_merge($codeInterfaceFiles));
    ...
    {
        $codeFiles = $this->getFiles(BP , '*.php');
        $phpFiles = [];
        $filesToCheck = $this->blacklistFilter(array_merge($codeFiles));
        foreach ($filesToCheck as $file) {
    ...
    protected function getFiles($dir, $pattern)
    {
        ...
        foreach (glob($dir . '/[!vendor]*', GLOB_ONLYDIR | GLOB_NOSORT) as $newDir) {
